...
const createE2ETestApp = () => {
  ...
  app.post('/jobs/:jobId/apply', authenticate, (req: any, res: any) => {
    ...
    const existingApplication = applications.find(
      (a) => a.jobId === jobId && a.applicantId === req.user.id,
    );
...
describe('StartupCompass Platform E2E Tests', () => {
  ...
  describe('Startup Management Flow', () => {
    test('should create and list startups', async () => {
      ...
      // List startups
      const listResponse = await request(app).get('/startups').expect(200);
    ...
    test('should filter startups by industry and stage', async () => {
      ...
      // Filter by industry
      const techResponse = await request(app).get('/startups?industry=Technology').expect(200);

      expect(techResponse.body.data.every((s: any) => s.industry === 'Technology')).toBe(true);

      // Filter by stage
      const seedResponse = await request(app).get('/startups?stage=seed').expect(200);
  ...
  describe('Job Board Flow', () => {
    test('should create job and handle applications', async () => {
      ...
      // List jobs
      const listJobsResponse = await request(app).get('/jobs').expect(200);
    ...
    test('should filter jobs by location and type', async () => {
      ...
      // Filter by location
      const remoteResponse = await request(app).get('/jobs?location=Remote').expect(200);

      expect(remoteResponse.body.data.every((j: any) => j.location.includes('Remote'))).toBe(true);

      // Filter by type
      const fullTimeResponse = await request(app).get('/jobs?type=full-time').expect(200);

      expect(fullTimeResponse.body.data.every((j: any) => j.type === 'full-time')).toBe(true);
    });
  });
  ...
  describe('Funding Tracking Flow', () => {
    test('should create and track funding rounds', async () => {
      ...
      // List funding rounds
      const listFundingResponse = await request(app).get('/funding').expect(200);
    ...
    test('should filter funding by round and amount', async () => {
      ...
      // Filter by round
      const seedResponse = await request(app).get('/funding?round=Seed').expect(200);

      expect(seedResponse.body.data.every((f: any) => f.round === 'Seed')).toBe(true);

      // Filter by amount range
      const bigRoundsResponse = await request(app).get('/funding?minAmount=5000000').expect(200);

      expect(bigRoundsResponse.body.data.every((f: any) => f.amount >= 5000000)).toBe(true);
    });
  });
  ...
  describe('Cross-Service Integration', () => {
    ...
    test('should handle pagination across all services', async () => {
      ...
      const startupsResponse = await request(app).get('/startups?page=1&limit=5').expect(200);
      ...
      const jobsResponse = await request(app).get('/jobs?page=1&limit=3').expect(200);
      ...
      const fundingResponse = await request(app).get('/funding?page=1&limit=2').expect(200);
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    test('should handle unauthorized access properly', async () => {
      ...
      const response = await request(app).post('/startups').send(startupData).expect(401);
  ...
  describe('Performance and Load Testing', () => {
    test('should handle concurrent requests across services', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const jobData = TestHelpers.generateTestData('job');
        promises.push(
          request(app).post('/jobs').set('Authorization', `Bearer ${authToken}`).send(jobData),
        );
      }
